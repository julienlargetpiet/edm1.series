...
#' @param step is the base step for the time indexes, step of one year (or any time unit), two year (or any time unit)?
#'
#' @examples
#'
#' set.seed(123)
...
sequence_na_med1 <- function(inpt_datf, bf_, step = 1){
...
sequence_na_mean1 <- function(inpt_datf, bf_, step = 1){
  for (I in 1:((ncol(inpt_datf) - 2) / bf_)){
    for (i in 1:bf_){
      cur_col <- ((I - 1) * bf_ + 2 + i)
      cur_mtch <- match(x = TRUE, table = is.na(inpt_datf[, cur_col]))
...
sequence_na_mean2 <- function(inpt_datf, bf_, step = 1){
  bf_ = bf_ + 1
  for (I in 1:((ncol(inpt_datf) - 2) / bf_)){
    for (i in 1:bf_){
      cur_col <- ((I - 1) * bf_ + 2 + i)
